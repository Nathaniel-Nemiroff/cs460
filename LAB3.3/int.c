...
//==> WRITE CODE TO GET get syscall parameters a,b,c,d from ustack 
   ...
   switch(a){
       ...
       case 30 : r = kpipe();	       break;
       case 31 : r = read(b, c, d);  break;
       case 32 : r = write(b, c, d); break;
       case 33 : r = close(b);    break;
       case 34 : r = pfd();	       break;
...
int kps()
{
	...
	printf("PROC %s(%d) STATUS : %s\n", running->name,running->pid, stat);
...
int read(int b, char *c, int d)
{
	printf("blah%s\n", c);
	read_pipe(b, c, d);
	return 0;
}
int write(int b, char *c)
{
	int i,j;
	for(i=0;i<PSIZE;i++){
		c[i]=get_byte(running->uss, c+i);
		if(c[i]==0){
		   j=i;
		   break;
		}
