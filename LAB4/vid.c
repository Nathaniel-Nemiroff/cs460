...
int scrollchk;		  // check for scroll
...
// added functions putcAtLoc() and printAtLoc()
putcAtLoc(char c, int x, int y)
{
   int savedrow = row, savedcol = column;
   row = y;
   column = x;
   scrollchk = 0;
   putc(c);
   ...
   row = savedrow;
   column = savedcol;
   set_VDC(CURSOR, (org + 2*(row*80 + column)) & vid_mask >> 1);
}
printAtLoc(char *c, int x, int y)
{
   int savedrow = row, savedcol = column;
   row = y;
   column = x;
